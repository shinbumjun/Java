...
import test.dept.dto.DeptDto2;
import test.util.DBConnect;

public class DeptDao2 {
   //부서정보를 추가하는 메소드(insert)
   public boolean insert(DeptDto2 dto) {
   ...
   public boolean update(DeptDto2 dto) {
      //필요한 값을 담을 지역 변수 미리 만들기
      Connection conn = null;
      PreparedStatement pstmt = null;
      int flag = 0;
   ...
   public DeptDto2 select(int deptno) {
   ...
   public List<DeptDto2> selectAll(){
      //부서 목록을 담을 ArrayList 객체를 생성해서 참조값을 지역변수에 담아 놓는다.
      List<DeptDto2> list=new ArrayList<>();
