...
public class DeptFrame extends JFrame 
	...
	public DeptFrame(String title) {
		super(title); //부모생성자에 다가 전달
		
		//프레임의 레이아웃 법칙 지정하기
		setLayout(new BorderLayout()); //3 this 생략 (총 15개)
		//상단 페널
		JPanel topPanel=new JPanel();
		topPanel.setBackground(Color.YELLOW); //3 topPanel.setBackground , 2 Color.YELLOW
		//페널을 상단에 배치하기 
		add(topPanel, BorderLayout.NORTH); // 3 add(topPanel, BorderLayout.NORTH), 2 BorderLayout.NORTH
		//페널에 추가할 UI 객체를 생성해서 
		...
		//테이블에 연결할 기본 모델 객체
		model=new DefaultTableModel(colNames, 0) { //익명클래스로 isCellEditable 오버라이드
			@Override
		...
		new UpdateThread().start(); //-1이 나오면 에러, 수정중에는 동작을 하지 않도록
	...
	public void printMember() {
		...
		for(MemberDto tmp:list) { 
			...
			//JTable에 출력할 row 하나의 정보를 Vector 객체에 담아서
			Vector<Object> row=new Vector<>(); //Vector는 Arraylist랑 비슷
			row.add(tmp.getNum());
			row.add(tmp.getName());
			row.add(tmp.getAddr());
	
			model.addRow(row);
			//model.addRow() object, Vector 둘다 사용가능
		}
	}
	...
	//***화면을 주기적으로 update 해주는 스레드, 내부 클래스
